/*
  # Catering Rental Booking System Schema

  ## Overview
  This migration creates the database structure for managing catering rental bookings
  ...
  ### `rentals`
  Tracks individual rental bookings
  - `id` (uuid, primary key) - Unique identifier for each rental booking
  ...
  - `status` (text, required) - Booking status: 'active', 'returned', 'overdue'
  - `notes` (text, optional) - Additional notes about the booking
  - `created_at` (timestamptz) - Record creation timestamp
  - `updated_at` (timestamptz) - Last update timestamp

  ### `rental_items`
  Stores individual items included in each booking
  ...
  - Add policies for authenticated users to manage booking data
  
  ## Important Notes
  1. All tables use UUID primary keys for better scalability
  2. Booking status defaults to 'active' for new bookings
